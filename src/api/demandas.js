...
export async function createDemanda(req, res) {
  ...
  try {
    ...
    notify({
      tipo: 'demanda_nova',
      titulo: `Nova demanda: ${demanda}`,
      detalhe: `${tipo} · ${prioridade || 'Normal'} · de ${solicitante || 'Equipe'} · roteia para ${sequencia}`,
      url: '/#demandas',
      para: 'mkt',
      marca: empresa && empresa !== 'AB Group geral' ? empresa : '',
    });
...
export async function updateDemandaStatus(req, res) {
  ...
  if (!row || !status) {
    return res.status(400).json({ error: 'row e status são obrigatórios' });
  }
