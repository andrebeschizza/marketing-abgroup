  ...
  try {
    ...
    let items = rows
      ...
      .map(r => ({
        ...
        status: r['Status'] || '',
        dataGravacao: r['Data Gravação'] || '',
        dataEnvioEdicao: r['Data Envio Edição'] || '',
        ...
        legenda: r['Legenda'] || '',
        linkRef1: r['Link Ref 1'] || '',
        linkRef2: r['Link Ref 2'] || '',
        linkRef3: r['Link Ref 3'] || '',
        notas: r['Notas'] || '',
...
export async function createCalendarioItem(req, res) {
  ...
  try {
    ...
    await appendRow(TAB, {
      ...
      'Data Gravação': body.dataGravacao || '',
      'Data Envio Edição': body.dataEnvioEdicao || '',
      ...
      'Link Ref 1': body.linkRef1 || '',
      'Link Ref 2': body.linkRef2 || '',
      'Link Ref 3': body.linkRef3 || '',
      'Notas': body.notas || '',
...
const FIELD_MAP = {
  ...
  dataGravacao: 'Data Gravação',
  dataEnvioEdicao: 'Data Envio Edição',
...
export async function updateCalendarioStatus(req, res) {
  ...
  try {
    ...
    // Hook: notifica MKT + a marca relacionada
    notify({
      tipo: 'calendario_movido',
      titulo: `${target['Título'] || 'Item'} → ${status}`,
      detalhe: `Movido de "${statusAnterior || '?'}" para "${status}"`,
      url: '/#calendario',
      para: 'mkt',
      marca: target['Marca'] || '',
    });
