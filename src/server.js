...
app.get('/healthz', async (req, res) => {
  const sheets = await ping();
  res.status(sheets.ok ? 200 : 503).json({ ok: sheets.ok, sheets, uptime: process.uptime() });
});

app.post('/login', (req, res) => {
  const { password } = req.body || {};
  const expected = process.env.AB_PASSWORD;
  if (!expected) return res.status(503).json({ error: 'Servidor sem AB_PASSWORD configurado' });
...
app.get('/api/me', (req, res) => {
  if (!req.session?.authenticated) return res.status(401).json({ authenticated: false });
  res.json({ authenticated: true, since: req.session.loggedAt });
});
...
app.get('*', (req, res) => {
  if (req.session?.authenticated) return res.sendFile(path.join(__dirname, '..', 'public', 'index.html'));
  return res.redirect('/login');
});
